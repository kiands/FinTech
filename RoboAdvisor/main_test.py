...
TOKEN = "1753648492:AAHJA7mz8UCTVicoizQbV8D5MNdSpIImfWE"
...
def start(update, _):
    ...
    # bot.send_message(update.message.chat.id, 'Hello, {}'.format(update.message.from_user.first_name), reply_to_message_id = update.message.message_id, reply_markup = ReplyKeyboardRemove())
    # bot.send_message(update.message.chat.id, '如果你想要開始投資基金，請輸入 /fund 我們會先為你做個投資風險評估\n如果你已經完成投資風險評估，請輸入 /result 看看我們為你推薦的基金\n', reply_to_message_id = update.message.message_id, reply_markup = ReplyKeyboardRemove())
...
def reply_handler(update, _):
    ...
    # print("now: ", Script_handler.qa_id[str(update.message.from_user.id)])
    if Script_handler.qa_id[str(update.message.from_user.id)] != -1:
        ...
        Script_handler.save_data()
        # print("!"*10)
        # print(Script_handler.answer_record)
        # print("!"*10)
        text = '作答完成\n'
        text += '✅NomuraBot已為你安排最適合的基金，想看看自己適合投資的基金，請輸入 /result\n'
        text += '🔄若想嘗試不同的投資方法，可以再次輸入 /fund，重新答題喔！\n'

        # update.message.reply_text('作答完成\n', reply_markup=ReplyKeyboardRemove())
        # update.message.reply_text('✅NomuraBot已為你安排最適合的基金，想看看自己適合投資的基金，請輸入 /result\n', reply_markup=ReplyKeyboardRemove())
        # update.message.reply_text('🔄若想嘗試不同的投資方法，可以再次輸入/fund，重新答題喔！\n', reply_markup=ReplyKeyboardRemove())
        update.message.reply_text(text, reply_markup=ReplyKeyboardRemove())

        # bot.send_message(update.message.chat.id, '作答完成\n', reply_to_message_id = update.message.message_id, reply_markup = ReplyKeyboardRemove())
        # bot.send_message(update.message.chat.id, '✅NomuraBot已為你安排最適合的基金，想看看自己適合投資的基金，請輸入 /result\n', reply_to_message_id = update.message.message_id, reply_markup = ReplyKeyboardRemove())
        # bot.send_message(update.message.chat.id, '🔄若想嘗試不同的投資方法，可以再次輸入/fund，重新答題喔！\n', reply_to_message_id = update.message.message_id, reply_markup = ReplyKeyboardRemove())
        return ConversationHandler.END
...
def record_handler(update, _):
    ...
    if update.message.text == "是":
        ...
        update.message.reply_text(text)
        # update.message.reply_text('那我們重新開始吧!')
        # update.message.reply_text('若中途想離開風險評估測試，請輸入 /cancel\n')
        # bot.send_message(update.message.chat.id, '那我們重新開始吧!', reply_to_message_id = update.message.message_id, reply_markup = ReplyKeyboardRemove())
        # bot.send_message(update.message.chat.id, '若中途想離開風險評估測試，請輸入 /cancel\n', reply_to_message_id = update.message.message_id, reply_markup = ReplyKeyboardRemove())
        Script_handler.reset_pointer(update.message.from_user.id)
        now_id = Script_handler.get_pointer(update.message.from_user.id)
        ask_question(update, _)
    ...
    if len(update.message.text) <= 6:
        ...
        if fund_text == None:
            ...
            if len(period_list) == 2 or len(period_list) == 1:
                ...
                text = text +  "💰投資期間最大漲幅：" + str(profit[0]) + "%\n"
                text = text +  "💰投資期間最大跌幅：" + str(profit[1]) + "%\n"
            else:
...
def error_handler(update, context: CallbackContext):
    ...
    # bot.send_message(update.message.chat.id, "抱歉，目前NomuraBot出了點小問題，請稍等一下", reply_to_message_id = update.message.message_id, reply_markup = ReplyKeyboardRemove())
...
def result(update, _):
    ...
    # bot.send_message(update.message.chat.id, '正在幫你評估適合的基金...', reply_to_message_id = update.message.message_id, reply_markup = ReplyKeyboardRemove())
    bot.send_chat_action(chat_id = update.message.chat_id, action = telegram.ChatAction.TYPING)
    time.sleep(1)
    global Script_handler
    global Econ_handler
    ...
    if answer_record != None:
        # update.message.reply_text(str(answer_record), reply_markup=ReplyKeyboardRemove())
        risk_result = Script_handler.risk_judge(update.message.from_user.id)
        recommend_fund = Econ_handler.fund_evaluation()
        recommend_list = Econ_handler.fund_filter(risk_result)
        recommend_list = Econ_handler.get_short_recommend_list(recommend_list)
        ...
        update.message.reply_text(fund_text, reply_markup=ReplyKeyboardRemove())
        # update.message.reply_text("你可以輸入 /more 查看完整的基金推薦清單喔", reply_markup=ReplyKeyboardRemove())
        # update.message.reply_text("你可以輸入 /info +你想要查詢的基金名稱來看看該基金的詳細資訊\n(小叮嚀：中間要空一格喔)", reply_markup=ReplyKeyboardRemove())
        # bot.send_message(update.message.chat.id, "🚩NomuraBot經過計算和評估後，認為你的風險可承受程度為： RR"+str(risk_result)+"\n"+reply_list[risk_result-1], reply_to_message_id = update.message.message_id, reply_markup = ReplyKeyboardRemove())
        # bot.send_message(update.message.chat.id, fund_text, reply_to_message_id = update.message.message_id, reply_markup = ReplyKeyboardRemove())
        # bot.send_message(update.message.chat.id, "你可以輸入 /more 查看完整的基金推薦清單喔", reply_to_message_id = update.message.message_id, reply_markup = ReplyKeyboardRemove())
        # bot.send_message(update.message.chat.id, "你也可以輸入 /info +你想要查詢的基金名稱來看看該基金的詳細資訊\n(小叮嚀：中間要空一格喔)", reply_to_message_id = update.message.message_id, reply_markup = ReplyKeyboardRemove())
        ...
        # bot.send_message(update.message.chat.id, "你尚未完成風險評估量表，請輸入 /fund 完成風險評估", reply_to_message_id = update.message.message_id, reply_markup = ReplyKeyboardRemove())
...
if __name__ == '__main__':
    ...
    # app.run(host='0.0.0.0',ssl_context=("fullchain.pem","privkey.pem"),port='443')

    #https://b110adece004.ngrok.io
    #https://api.telegram.org/bot1753648492:AAHJA7mz8UCTVicoizQbV8D5MNdSpIImfWE/setWebhook?url=https://b110adece004.ngrok.io/hook